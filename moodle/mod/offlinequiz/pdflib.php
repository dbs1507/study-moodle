...
class offlinequiz_answer_pdf extends offlinequiz_pdf {
    ...
    public function set_user($user) {
        ...
        // var_dump($user);
        // die();
        //qualquer coisa dar um vardump e die
    }
    ...
    public function Header() {
        ...
        if(empty($this->user)){
            $this->Cell(90, 7, ' '.offlinequiz_str_html_pdf(get_string('profile_field_serie',  'offlinequiz')).":", 1, 1, 'L');
            $this->Cell(90, 7, ' '.offlinequiz_str_html_pdf(get_string('profile_field_turma',  'offlinequiz')).":", 1, 1, 'L');

        }else{
            $this->Cell(45, 7, ' '.offlinequiz_str_html_pdf(get_string('profile_field_serie', 'offlinequiz')).": " . $this->user->profile_field_serie, 1, 0, 'L');
            $this->Cell(45, 7, ' '.offlinequiz_str_html_pdf(get_string('profile_field_turma', 'offlinequiz')).": " . $this->user->profile_field_turma, 1, 1, 'L');
            // Desenha a linha vertical entre "Serie" e "Turma"
            $this->Line(56, $this->GetY() - 7, 56, $this->GetY());
        }
        $this->Ln(5);
        ...
        for ($i = 0; $i <= 5; $i++) {
            ...
            if (!empty($this->group) and $letterstr[$i] == $this->group) {
                $this->Image("$CFG->dirroot/mod/offlinequiz/pix/ausstreichen.jpg", $this->GetX() - 2.75,  $this->Gety() + 0.15,  3.15,  0);
            }
        }
        ...
        $this->Image("$CFG->dirroot/mod/offlinequiz/pix/ausstreichen.jpg",  57.2,  78.2,  3.15,  0);   // JZ added 0.4 to y value.
        $this->Image("$CFG->dirroot/mod/offlinequiz/pix/spacer.gif", 56.8,  93,  4.1,  0);  // JZ added 0.4 to y value.
        $this->SetY(93.1);
        $this->Cell(42, 8, "", 0, 0, 'C');
            ...
            for ($j = 0; $j <= 9; $j++) {
                ...
                if (!empty($this->user)) {
                    if ($this->user->{$offlinequizconfig->ID_field}[$i] == $j) {
                        $this->Image("$CFG->dirroot/mod/offlinequiz/pix/ausstreichen.jpg", $x,  $y + 0.15,  3.15,  0);
                    }
                }
            }
        }
